...
def test_chai_config_pins_chai_to_8001():
    chai = yaml.safe_load((ROOT / "configs" / "chai.yaml").read_text())
...
def test_chai_and_gelateria_are_distinct_merchants():
    c = yaml.safe_load((ROOT / "configs" / "chai.yaml").read_text())["merchant"]
    g = yaml.safe_load((ROOT / "configs" / "gelateria.yaml").read_text())["merchant"]
...
def test_money_config_defaults_are_integer():
    ...
    money_fields = [
        ...
        if name in config_mod.Settings.model_fields
        or any(
            name in cls.model_fields
            for cls in (
                config_mod.CampaignSettings,
                config_mod.MerchantConfig,
            )
        )
    ]
    for name in money_fields:
        owner = next(
            cls
            for cls in (config_mod.CampaignSettings, config_mod.MerchantConfig)
