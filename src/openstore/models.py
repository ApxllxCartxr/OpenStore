...
class Checkout(SQLModel, table=True):
    ...
    currency: str = Field(max_length=3, default="INR")
    state: OrderState = Field(default=OrderState.CREATED, sa_column=Column(SQLEnum(OrderState), nullable=False))
...
class IdempotencyKey(SQLModel, table=True):
    ...
    __table_args__ = (
        Index("ix_idempotency_trace_client", "trace_id", "client_id"),
    )
...
class WebhookEvent(SQLModel, table=True):
    ...
    status: WebhookStatus = Field(default=WebhookStatus.RECEIVED, sa_column=Column(SQLEnum(WebhookStatus), nullable=False))
...
class WebAuthnCredential(SQLModel, table=True):
    ...
    public_key: bytes = Field(sa_column=Column(LargeBinary, nullable=False))  # COSE-encoded public key
...
class Campaign(SQLModel, table=True):
    ...
    state: CampaignState = Field(default=CampaignState.DRAFT, sa_column=Column(SQLEnum(CampaignState), nullable=False))
