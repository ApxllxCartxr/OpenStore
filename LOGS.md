...
First baseline: 299 mutants, 191 killed, 99 survived, 9 no-tests. Two survivors were real, not noise: `_check_authority` computed a `Binding` and assigned it to `ctx.binding` on every synchronous (lands-before-gate) decision, but nothing ever read it — the real receipt-sealing path in `checkout.py` independently re-derives the binding from `authority_kind`/`mechanism` rather than trusting a value computed earlier, and `transcript.binding` is populated only by `settle()`'s separate deferred UPI-PIN path. Every mutation of that dead assignment survived because nothing could observe the difference; deleted the field. Second: the per-order cap check used `total_minor > cap` with no test at the boundary, so a mutant flipping it to `>=` survived silently — added one (a total sitting exactly on the cap must pass, "exceeds" means strictly over). `_first_difference`'s three message branches were also only ever exercised through a loose `X in detail or Y in detail` assertion; added a direct unit test pinning all three.

The other 97 survivors are an untriaged backlog — mostly message-text mutations on FAIL-path detail strings by a first read, not obviously more logic bugs, but that scan wasn't exhaustive. Recorded in memory (`mutmut_survivor_backlog.md`) rather than the old stale one. Next pass: widen test selection to `test_golden_replays.py` + `test_protocols.py` before trusting remaining "survived" verdicts, then triage what's left; after that, the same `source_paths`-covers-the-package pattern extends to `ledger/entries.py`.
