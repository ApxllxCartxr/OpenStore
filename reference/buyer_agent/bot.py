...
from reference.buyer_agent.discover import discover_merchant, discover_auth_server
from reference.buyer_agent.oauth_client import (
    generate_pkce_pair,
    LoopbackServer,
    open_authorize_url,
...
from reference.buyer_agent.graph import build_graph, ConversationState
from reference.buyer_agent.intent import ensure_intent_policy_signed
...
async def on_message(message: discord.Message) -> None:
    ...
    # Policy check first — needs no token, so the bot replies promptly.
    if conversation_id not in _signed_policy_cache:
        ...
        try:
            ...
            await message.channel.send(f"Could not check Intent Policy: {e}")
            return
        if already_signed:
    ...
    # Acquire a merchant token (non-blocking; raises clearly if none cached).
    try:
        token = await _get_or_create_token()
    except Exception as e:
        await message.channel.send(
            f"Authorize with the merchant to continue (see printed authorize URL): {e}"
        )
        return
...
def _acquire_token_blocking() -> str:
    """Discover merchant, register OAuth client, complete the interactive
    loopback redirect, and return a cached access token.

    Runs in a worker thread (see ``_get_or_create_token``) so the bot's event
    loop stays responsive while waiting on the browser redirect.
    """
...
async def _get_or_create_token() -> str:
    """Run the (blocking) OAuth flow in a worker thread so the bot stays up."""
