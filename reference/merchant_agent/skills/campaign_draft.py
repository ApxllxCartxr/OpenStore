...
def campaign_draft_skill(occasion_tag: str) -> dict:
    ...
    merchant = _catalog.merchant_name
    draft = (
        f"Celebrate with {merchant}! For your {occasion_label.lower()} "
        f"occasion, try our {product_list} — crafted with authentic recipes "
        f"and the finest ingredients. Order now for a sweet celebration!"
    )
