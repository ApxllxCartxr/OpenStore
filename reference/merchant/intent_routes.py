...
from reference.merchant.webauthn import (
...
def intent_status(user_id: str = "default-user", session: Session = Depends(get_session)):
    row = session.exec(
        ...
        .where(IntentPolicyRow.user_id == user_id)
        .order_by(IntentPolicyRow.created_at.desc())
    ).first()
    return {"has_policy": row is not None, "credential_id": row.credential_id if row else None}
...
def issue_webauthn_challenge(
    ...
    # Run the fido2 registration begin so the server holds the challenge state
    # that register_complete requires (the authentication flow does the same
    # via state_json). The browser's own options use this same nonce as the
    # challenge, so the ceremony verifies end-to-end.
    user = PublicKeyCredentialUserEntity(
        id=os.urandom(16), name="openstore-user", display_name="OpenStore User",
    )
    _, state = fido_server.register_begin(user, challenge=nonce_bytes)
...
def begin_signing(
    ...
    # fido2's options object carries raw bytes/enums that FastAPI cannot
    # JSON-encode. Return a plain, JSON-safe dict the browser can consume.
    ...
    return {"nonce": nonce_b64, "options": safe_options}
...
def complete_signing(
    ...
    # Resolve the credential from the assertion's rawId when present, so signing
    # works no matter which registered passkey the user selects.
    raw_id = assertion.get("rawId")
    row = None
    if raw_id:
        row = session.exec(
            select(IntentPolicyRow)
            .where(IntentPolicyRow.credential_id == raw_id)
            .where(IntentPolicyRow.active == True)
        ).first()
    if row is None and credential_id:
        row = session.exec(
            select(IntentPolicyRow)
            .where(IntentPolicyRow.credential_id == credential_id)
            .where(IntentPolicyRow.active == True)
