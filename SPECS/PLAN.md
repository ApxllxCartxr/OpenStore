...
- **`spoiledduckie.localhost`** — a real accessory shop. Home, shop with filters, product-group pages with a colour/size picker, a direct checkout that takes no money, order lookup by token, GST and shipping on every total, `ProductGroup` + `hasVariant` JSON-LD on every group page.
...
Trimmed by depth: CSV import (export only), bulk archive, multi-image galleries (one hero + two alts, generated), dispute UI (the Ledger kind exists, the console does not), retention scheduler (the erasure button exists, the cron does not), Prometheus (counters on `/healthz`, no scrape), the seed *list* reader beyond direct-add + one custom URL.
...
10. `quote-consistent` — lines sum to total, signs correct, currency matches, subtotal equals `sum(qty × attested price)` from the pinned Attestation, tax additive-or-informational per the flag, delivery and discounts apportioned by taxable value with largest-remainder, half-up to the paise, any rupee rounding present as an explicit `round_off` line. Checking a Merchant's sums is not computing prices — without it a buggy or compromised Merchant gets its total signed unchallenged.
...
**Storefront**: home (hero, featured, search) · shop (category/tag/price/availability filters, ordered results, pagination) · **group page** (gallery, picker over the group's axes, add-ons, where picking resolves to exactly **one** Catalogue Item before `Add` is enabled, showing its own price and its own `in-stock/low-stock/sold-out`, with unavailable combinations shown as unavailable) · direct cart + checkout (**takes no payment** — a second money path is forbidden; a direct order is created when the shop confirms it and is marked paid by hand in admin, exactly the "recorded manually for direct buyers" case the spec already names; the storefront's own cart holds stock for 30 min before any order row exists, so an abandoned browser cart cannot leak inventory) · order lookup **by unguessable token only**, with the human order number additionally requiring a matching Contact Point under a throttle — a guessable lookup key is an IDOR that hands strangers other people's addresses.
...
- **DONE WHEN**: store boots alone; seed loads with zero null-stock rows; catalogue validation rejects missing/negative/float stock or a missing GST rate or HSN with a **named** error; home → shop → group → resolved variant → direct checkout (with shipping and GST) → lookup works with zero sidecar imports; the unavailable combination renders as unavailable.

### B2 · The nine doors — 3h

Implemented natively over HTTP on the private network, per §6.1.

The three pricing rules that decide whether the invoice is legal — pinned **here**, because two implementations that guess differently trip the Gate's `quote-inconsistent` check:
...
### B3 · Shop-ops admin — 3.0h *(3.5h less 0.5h by using a headless table/form primitive for the variant matrix rather than hand-rolling one — the design system supplies the styling either way)*
...
Freeze §6 and commit it: the nine doors, the Quote shape, the `cart_hash` preimage **with its golden vector**, `core/codes.py` with every closed set, and the generated `docs/CODES.md`. **Nothing else starts until this is committed.** These are the last things that change cheaply, and everything downstream pins their bytes.

Also in this commit: the one-line `SPEC.md §1` / `PLAN-merchant-site.md` amendment for **D1** (SvelteKit), since a contracts commit is where spec edits belong.
