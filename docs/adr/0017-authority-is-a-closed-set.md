...
Standard UPI has no authorize-then-capture, which is the real reason COD sat outside v1 (SPEC §13) — escrow-zero could not model a hold that becomes cash days later at a doorstep. Reserve Pay blocks funds, which is that primitive, on a rail NPCI already runs: block at order (`RESERVE`), capture at delivery (`CAPTURE`), release on RTO (`RELEASE`), inside its published caps. Above the cap, or for true cash, `confirmed-intent` carries the order and the Ledger moves no money until delivery.

This ADR only makes that path expressible. The path itself — the deadlines, the sweep, what an undelivered block does at expiry, how RTO reconciles — is a money-core change and needs its own ADR before any of it is built.
